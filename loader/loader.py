...
class DatasetSerial(RNGDataFlow):
    ...
    Produce ``(image, label)`` pair, where 
        ...
        ``label`` is a float image of shape (H, W, C). Number of C depends
                  ...
                  If self.model_mode is 'sonnet': 
                    channel 0 binary nuclei map, values are either 0 (background) or 1 (nuclei)
                    channel 1 containing the type map 
                    channel 2 containing the ordinal map
    """
...
#### 
def valid_generator(ds, shape_aug=None, input_aug=None, label_aug=None, batch_size=8, nr_procs=1):
    ...
    # ds = DataThread(batch_size, ds)
    # ds = QueueInput(ds)
    return ds

####
def train_generator(ds, shape_aug=None, input_aug=None, label_aug=None, batch_size=4, nr_procs=4):
    ### augment both the input and label
    ds = ds if shape_aug is None else AugmentImageComponents(ds, shape_aug, (0, 1), copy=True)
